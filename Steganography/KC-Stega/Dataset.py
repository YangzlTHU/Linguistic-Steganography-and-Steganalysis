...
class EPWDataset:
    ...
    def prepare_dataset(self):
        with self.input_graph.as_default():
            ...
            #self.test = self.get_dataset(test_file, False)
    ...
    def process_inst(self, line):
        res = {"feats" + suffix: [] for suffix in ['_key', '_val', '_valword']}
        ...
        for item in feats:
            ...
            res["feats_valword"].append(self.vocab.lookup(item[1], 2))
        ...
        val_word = [self.vocab.lookup("<ADJ>", 2)] + res['feats_valword']
        ...
        return (
            np.array(key_input, dtype=np.int32), np.array(val_input, dtype=np.int32), np.array(val_word, dtype=np.int32),
    ...
    def get_dataset(self, filename, train=True):
        ...
        dataset = dataset.map(map_func=lambda x: tf.py_func(lambda y: process(y), [x], Tout=[tf.int32] * 15))
        ...
        def key_func(p_1, p_2, p_20, input_len,
                     p_3, p_4, p_5,
                     p_6, p_7, gcnt,
                     p_8, p_9,
                     p_10, slen,
